    ...
    def movement(self):
        ...
        if not x == 0 or not y == 0:
            ...
            if pins.layer_2_key.value:
                mouse.move(0, 0, round(-x*0.13))
                if abs(round(-y*0.13)) > 0:  # try to kill horizontal scroll when not used, as not to spam shift
                    pins.kbd.press(Keycode.SHIFT)
                    mouse.move(0, 0, round(-y*0.13))
                    pins.kbd.release(Keycode.SHIFT)
            else:
                mouse.move(y, x, 0)
            self.is_moving = True
        else:
